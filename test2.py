...
def get_p_r_f(pred,true):
    p_macro, r_macro, f_macro, support_macro  = \
        precision_recall_fscore_support(pred, true, average='macro',zero_division=1)
    p_w, r_w, f_w, support_w = \
        precision_recall_fscore_support(pred, true, average='weighted', zero_division=1)
    return p_macro,r_macro,f_macro,f_w



def evaluate(model, dataset, device):

    # data_set = MET(data_split=data_split)
    data_set = dataset
    ...
    with torch.no_grad():
        ...
        p, r, f1, w_f1 = get_p_r_f(y_predicted, y_true)
