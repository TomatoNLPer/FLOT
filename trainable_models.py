...
import numpy as np
from trainable_OT.otk.models_deepsea import OTLayer
...
class HateModel(nn.Module):
    def __init__(self, scratch=True, class_num = 3, gamma=0.7,pretrained_weights_path = '../clip-vit-base-patch32'):
        ...
        self.criterion = ContrastiveLoss(margin=0.1)
        self.gamma = gamma
        if class_num == 2:
    ...
    def forward(self, img, input_ids, att_mask):
        ...
        image_embeds = self.img_trans(pooled_output)
        ...
        sim_logit, sim_vec = self.similarity_module(patches,text_seq,img_embed,text_embed,cap_lens = 77)
        sim_loss = self.criterion(sim_vec)
        ...
        score = gamma * logits + (1 - gamma) * sim_logit
        # score = logits
        # score = gamma * logits + (1 - gamma) * sim_vec
        # for cross_attention
        # score = gamma * logits + (1 - gamma) * sim_vec.squeeze(dim=1)
        # return logits
        return score,sim_loss

    def compute_clip_alignment(self, img, input_ids, att_mask):
        with torch.no_grad():
        # patches, img_embed= self.pre_model.get_image_features(img,output_hidden_states = True, return_dict=True)
            ...
            alignment = torch.matmul(text_embed, img_embed.t()).diag()
            return alignment
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, scores):
        ...
        # caption retrieval
        cost_s = (self.margin + scores - d1).clamp(min=0)/len(diagonal)
        # compare every diagonal score to scores in its row
        # image retrieval
        cost_im = (self.margin + scores - d2).clamp(min=0)/len(diagonal)
...
class EncoderSimilarity(nn.Module):
    ...
    def __init__(self, embed_size, sim_dim,class_num, module_name='SGR', sgr_step=4):
        ...
        self.sim_w = nn.Linear(class_num, 1)
        self.sigmoid = nn.Sigmoid()

        #best_settings:max_iter = 5
        self.text2img_ot = OTLayer(in_dim=512, out_size=50, heads=2, eps=0.1, max_iter=3,\
                                   position_encoding=None, position_sigma=0.1, out_dim=None,\
                                   dropout=0.4)
        self.img2text_ot = OTLayer(in_dim=512, out_size=50, heads=2, eps=0.1, max_iter=3,\
                                   position_encoding=None, position_sigma=0.1, out_dim=None,\
                                   dropout=0.4)
    ...
    def forward(self, img_emb, cap_emb, img_g,text_g, cap_lens):
        ...
        context_img = self.text2img_ot(img_emb)
        context_text = self.img2text_ot(cap_emb)
        n_image = context_img.size(0)
        n_caption = context_text.size(0)
        ...
        for i in range(n_caption):
            ...
            # n_word = cap_lens[i]
            n_word = cap_lens
            cap_i = context_text[i, :n_word, :].unsqueeze(0)
            ...
            img_i = context_img[i].unsqueeze(dim = 0)
            # local-global alignment construction
            sim_img = SCAN_attention(cap_i_expand, context_img, smooth=9.0)

            # for heuristic
            # sim_loc1 = torch.pow(torch.sub(Context_img, cap_i), 2)
            # sim_loc2 = torch.pow(torch.sub(Context_text, img_i), 2)
            sim_rep = torch.pow(torch.sub(sim_img, cap_i), 2)
            sim_loc = l2norm(self.sim_tranloc_w(sim_rep), dim=-1)
            ...
            # sim_s = self.sigmoid(self.sim_eval_w(sim_vec))[i]
            # for cross attention
            sim_i = self.sigmoid(self.sim_eval_w(sim_vec))

            # sim_s_all.append(sim_s)
        ...
        sim_all = torch.stack(sim_all, 0)
        sim_logit = torch.diagonal(sim_all).transpose(1,0)
        sim_all = self.sim_w(sim_all).squeeze(dim=-1)

        return sim_logit, sim_all
