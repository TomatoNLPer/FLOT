...
trainset = FineHarmP(data_split='train')
testset = FineHarmP(data_split='test')
validset = FineHarmP(data_split='val')
...
model = HateModel(scratch = True, class_num=3, gamma=0.5).to(device)
...
for epoch in range(num_epoch):
    ...
    acc, f1, w_f1 = evaluate(model, 'test', device)
